...
train_labels_80, train_num_each_80, train_start_vidx,\
    ...
    test_labels_80, test_num_each_80, test_start_vidx = get_data(common.PKL_DATA) # CHANGED: common
...
with open(common.LFB_TRAIN, 'rb') as f: # CHANGED: common
    g_LFB_train = pickle.load(f)

with open(common.LFB_VAL, 'rb') as f: # CHANGED: common
    g_LFB_val = pickle.load(f)

with open(common.LFB_TEST, 'rb') as f: # CHANGED: common
...
# CHANGED: weights from load_pkl.py
...
criterion_phase = nn.CrossEntropyLoss(weight=torch.from_numpy(common.weights_train).float().to(device)) # CHANGED: common
...
model_name = common.TECNO_MODEL # CHANGED: common
...
train_we_use_start_idx_80 = [x for x in range(5)] # CHANGED
val_we_use_start_idx_80 = [x for x in range(1)] # CHANGED
test_we_use_start_idx_80 = [x for x in range(1)] # CHANGED
...
for epoch in range(max_epochs):
    ...
    for i in train_we_use_start_idx_80:
        ...
        # p_classes1 = common.transition_probability(p_classes1) # added
    ...
    with torch.no_grad():
        for i in val_we_use_start_idx_80:
            ...
            # p_classes = common.transition_probability(p_classes) # added
    ...
    if val_accuracy_phase > best_val_accuracy_phase:
        ...
        # CHANGED: added
        np_labels = np.array(test_all_labels_phase).reshape(7974, 1)
        np_preds = np.array(test_all_preds_phase).reshape(7974, 1)
        np_data = np.concatenate((np_labels, np_preds), 1)
        np.savetxt(common.SAVE_RESULT_CSV, np_data, delimiter=",", fmt="%d")
