...
import random
import numbers
# from torch.utils.tensorboard import SummaryWriter # CHANGED: commentout
from sklearn import metrics

import common # CHANGED: added
...
parser.add_argument('--in_ch', default=3, type=int, required=True, help='define input channel') # CHANGED: add
...
def pil_gray(np_img):
    ...
    return img.convert('L') # 8bit gray scale
...
class RandomCrop(object):
    ...
    def __call__(self, img):
        ...
        # print(is_mask, self.count, x1, y1, tw, th)
        # print("channel: ", img.getbands())
...
class CholecDataset(Dataset):
    ...
    def __init__(self, file_paths, file_labels, transform=None, loader=pil_loader, in_ch=in_ch): # CHANGED
        self.file_paths = file_paths
        self.file_labels_phase = file_labels[:, 0]
        self.transform = transform
        self.loader = loader
        self.in_ch = in_ch # CHANGED: add
    ...
    def __getitem__(self, index):
        ...
        # CHANGED: add mask
        if self.in_ch == 6:
            mask_names = img_names.replace("org_imgs", "body_tissue")
            np_mask = cv2.imread(mask_names)
            masks = list()
            for ch in range(self.in_ch - 3):
                masks.append(pil_gray(np_mask[ch]))
        elif self.in_ch == 14 or self.in_ch == 11:
            masks = list()
            bt_names = img_names.replace("org_imgs", "body_tissue")
            bts = cv2.imread(bt_names) # (270, 480, 3)

            tool_names = img_names.replace("org_imgs", "5class_tools")
            tool_names = tool_names.replace("png", "npz")
            tool_comp = np.load(tool_names)
            tools = tool_comp["arr_0"] # (270, 480, 8)
            if self.in_ch == 11:
                tools = tools[:, :, 2:] # remove forceps and tweezers
                scalpels_tools = tools[:, :, :2] # e_scalpel and scalpel
                others_tools = tools[:, :, 3:] # e_scalpel and scalpel
                tools = np.concatenate([scalpels_tools, others_tools], axis=2)

            np_masks = np.concatenate((bts, tools), 2)

            for ch in range(self.in_ch - 3):
                masks.append(pil_gray(np_masks[ch]))

        if self.transform is not None:
            imgs = self.transform(imgs)
            # CHANGED: add below
            if self.in_ch == 6 or self.in_ch == 14 or self.in_ch == 11:
                np_img = np.array(imgs)
                for mask in masks:
                    mask = self.transform(mask)
                    np_mask = np.array(mask)
                    np_img = np.concatenate((np_img, np_mask), 0)
                imgs = torch.from_numpy(np_img).clone() # (ch, 224, 224)
                # masks = self.transform(masks)
                # np_img = np.array(imgs)
                # np_mask = np.array(masks)
                # np_concat = np.concatenate((np_img, np_mask), 0)
                # imgs = torch.from_numpy(np_concat).clone() # (6, 224, 224)
                # # tensor_concat = tensor_concat.unsqueeze(0).permute(0, 3, 1, 2)
...
class resnet_lstm(torch.nn.Module):
    # def __init__(self):
    def __init__(self, in_ch): # CHANGED
        super(resnet_lstm, self).__init__()
        self.in_ch = in_ch # CHANGED: add
        resnet = models.resnet50(pretrained=True)
        resnet.conv1 = nn.Conv2d(
                in_ch, 64, kernel_size=7, stride=2, padding=3, bias=False) # CHANGED: added
        ...
        self.fc = nn.Sequential(nn.Linear(2048, 512),
                                nn.ReLU(),
                                nn.Linear(512, 6)) # CHANGED: maybe 7-> 28
    ...
    def forward(self, x):
        ...
        x = x.view(-1, self.in_ch, 224, 224) # CHANGED
        ...
        #y = self.dropout(y)
        y = self.fc(x)
        # y = y.view(y.size()[0], 4, 7) # commentout
        return y
...
def get_data(data_path):
    ...
    train_dataset_80 = CholecDataset(train_paths_80, train_labels_80, train_transforms, in_ch=in_ch) # CHANGED
    val_dataset_80 = CholecDataset(val_paths_80, val_labels_80, test_transforms, in_ch=in_ch) # CHANGED
    test_dataset_80 = CholecDataset(test_paths_80, test_labels_80, test_transforms, in_ch=in_ch) # CHANGED
...
def valMinibatch(testloader, model):
    ...
    with torch.no_grad():
        ...
        for data in testloader:
            ...
            inputs = inputs.view(-1, sequence_length, self.in_ch, 224, 224) # CHANGED
...
def train_model(train_dataset, train_num_each, val_dataset, val_num_each):
    # TensorBoard
    # writer = SummaryWriter('runs/emd_lr5e-4/') # commentout
    ...
    # model = resnet_lstm(in_ch)
    model = resnet_lstm(in_ch) # CHANGED: arg in_ch
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_loader_80):
            ...
            # inputs = inputs.view(-1, sequence_length, 3, 224, 224)
            inputs = inputs.view(-1, sequence_length, in_ch, 224, 224) # CHANGED
            outputs_phase = model.forward(inputs)
            outputs_phase = outputs_phase[sequence_length - 1::sequence_length]

            _, preds_phase = torch.max(outputs_phase.data, 1)
            # outputs_phase, _ = torch.max(outputs_phase.data, 1) # CHANGED: outputs_phase 
            # _, preds_phase = torch.max(outputs_phase.data, 1) # CHANGED: added
            # outputs_phase.requires_grad_(True) # CHANGED: added
            loss_phase = criterion_phase(outputs_phase, labels_phase)
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                _, preds_phase = torch.max(outputs_phase.data, 1) #CHANGED: commentout
                # outputs_phase, _ = torch.max(outputs_phase.data, 1) # CHANGED: outputs_phase 
                # _, preds_phase = torch.max(outputs_phase.data, 1) # CHANGED: added
                # outputs_phase.requires_grad_(True) # CHANGED: added
                loss_phase = criterion_phase(outputs_phase, labels_phase)
        ...
        torch.save(best_model_wts, "./best_model/emd_lr10e-5/" + base_name + ".pth")
        print("best_epoch", str(best_epoch))

        # CHANGE: add
        train_loss.append(train_average_loss_phase)
        valid_loss.append(val_average_loss_phase)
        test_loss.append(test_average_loss_phase)
        torch.save({
            "epoch" : epoch,
            "train_loss" : train_loss,
            "valid_loss" : valid_loss,
            "test_loss" : test_loss
            # }, "./best_model/emd_lr5e-4/loss.pth")
            }, "./best_model/train_emd_loss.pth")
...
def main():
    ...
    test_dataset_80, test_num_each_80 = get_data(common.PKL_DATA) #CHANGED: common 
