...
parser.add_argument('--in_ch', default=3, type=int, required=True, help='input channel')
...
class CholecDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.in_ch == 6:
            ...
            tool_names = img_names.replace("org_imgs", "tools")
            ...
            if self.in_ch == 11:
                ...
                tools = np.concatenate([scalpels_tools, others_tools], axis=2) # (270, 480, 5)
...
class resnet_lstm(torch.nn.Module):
    ...
    def __init__(self, in_ch): # CHANGED
        ...
        resnet.conv1 = nn.Conv2d(
                in_ch, 64, kernel_size=7, stride=2, padding=3, bias=False) # CHANGED
    ...
    def forward(self, x):
        ...
        x = x.view(-1, self.in_ch, 224, 224) # CHANGED: 3 -> 6
...
def get_data(data_path):
    ...
    train_dataset_80_LFB = CholecDataset(train_paths_80, train_labels_80, test_transforms, in_ch=in_ch) # CHANGED
    val_dataset_80 = CholecDataset(val_paths_80, val_labels_80, test_transforms, in_ch=in_ch) # CHANGED
    test_dataset_80 = CholecDataset(test_paths_80, test_labels_80, test_transforms, in_ch=in_ch) # CHANGE
...
def train_model(train_dataset, train_num_each, val_dataset, val_num_each):
    ...
    if not load_exist_LFB:
        ...
        model_LFB = resnet_lstm(in_ch) # CHANGED

        model_LFB.load_state_dict(torch.load(common.EMBED), strict=False) # CHANGED: common
        ...
        #'''
        with open(common.LFB_TRAIN, 'wb') as f: # CHANGED: chenged
            pickle.dump(g_LFB_train, f)

        with open(common.LFB_VAL, 'wb') as f: # CHANGED: chenged
            pickle.dump(g_LFB_val, f)
        #'''
        with open(common.LFB_TEST, 'wb') as f: # CHANGED: chenged
...
def main():
    train_dataset_80, train_num_each_80, \
            val_dataset, val_num_each, test_dataset, test_num_each = get_data(common.PKL_DATA) # CHANGED: common
